...
# Synthetic-footage guard. Veo/Sora/Kling/Runway uploads look plausible in search results but
# wreck the cut: no real continuity, dreamlike artefacts, and they never match the narration.
_SLOP_ALNUM = set("abcdefghijklmnopqrstuvwxyz0123456789")
# Token list, not a regex: these titles must be rejected before they poison the cut.
# Synthetic footage (Veo/Sora/Kling/Runway) looks plausible in search results but has no real
# continuity and never matches the narration.
AI_SLOP_TOKENS = (
    " veo ", " veo2 ", " veo3 ", " veo 2 ", " veo 3 ", " sora ", " kling ", " runway ",
    " runwayml ", " pika ", " pikalabs ", " midjourney ", " lumadream ", " haiper ",
    " ai generated ", " aigenerated ", " ai video ", " ai footage ", " ai film ",
    " ai movie ", " ai cinematic ", " ai animation ", " generated by ai ",
    " generated with ai ", " made with ai ", " text to video ", " prompt to video ",
    " img to video ", " image to video ",
)

def _slop_norm(x):
    """Lowercase and reduce every non-alphanumeric run to a single space, padded."""
    out = []
    prev_space = True
    for ch in x.lower():
        if ch in _SLOP_ALNUM:
            out.append(ch)
            prev_space = False
        elif not prev_space:
            out.append(" ")
...
def is_ai_slop(title, channel=""):
    t = _slop_norm(str(title) + " " + str(channel))
    return any(tok in t for tok in AI_SLOP_TOKENS)

MIN_VIEWS = int(os.environ.get("MIN_SOURCE_VIEWS", "20000"))
...
def discover_sources(segs, title=""):
    ...
    vids, seen_ch, rejected = [], set(), 0
    for qy in queries:
        r = run([sys.executable, "-m", "yt_dlp", *PX, "--flat-playlist",
                 "--print", "%(id)s|%(duration)s|%(view_count)s|%(channel)s|%(title)s", "--playlist-end", "8",
                 "--extractor-args", "youtube:player_client=web_safari,android",
                 f"ytsearch8:{qy}"], timeout=180)
        for line in r.stdout.strip().splitlines():
            ...
            if is_ai_slop(ttl, ch):                     # veo/sora/etc — synthetic b-roll ruins the cut
                rejected += 1; continue
            if views < MIN_VIEWS:                       # AI slop channels are overwhelmingly low-view
                rejected += 1; continue
            seen_ch.add(ch); vids.append(("auto", p[0]))
    print(f"  discovered {len(vids)} source videos across {len(seen_ch)} channels "
          f"({rejected} rejected as AI-generated/low-view)")
...
# ---------- 8a. deterministic overlay track (used when the AI director is unavailable) ----------
_NUM_UNITS = ("percent", "%", "million", "billion", "thousand", "people", "deaths", "dead",
              "injured", "missing", "miles", "km", "kilometers", "feet", "meters", "degrees",
              "magnitude", "years", "hours", "minutes", "dollars")
_STOP_HEAD = ("the", "a", "an", "and", "but", "so", "then", "that", "this", "it", "in", "on", "of")
...
def _find_number(text):
    """Return (value:str, unit:str) for the first meaningful figure in the text."""
...
def _enumerationish(text):
    """Segments that list things make good FactCard material."""
    t = str(text)
    return t.count(",") >= 2 or " and " in t.lower()

def auto_overlays(segs, total, max_overlays=40):
    """Build a spaced, varied overlay track straight from the narration."""
    ov = []
    if not segs:
        return ov

    def push(start, dur, component, props, sfx=""):
        ov.append({"start": round(float(start), 2), "dur": round(float(dur), 2),
                   "component": component, "props": props, "sfx": sfx})

    # 1. Hook on the opening line.
    push(0.6, 4.2, "HookCaption", {"lines": [_clean(segs[0]["text"], 80)]}, "riser")

    last_at = 0.6
    GAP = 13.0          # keep overlays from stacking on top of each other
    used_facts = 0

    for sg in segs:
        st = float(sg.get("start", 0))
        if st < 6 or st > max(0.0, total - 4):
            continue
        if st - last_at < GAP:
            continue
        text = sg.get("text", "")
        ...
        val, unit = _find_number(text)
        placed = False

        # 2. A figure in the line -> BigNumber (the eye goes straight to it).
        if val is not None:
            try:
                num = float(val)
            except ValueError:
            ...
            if num is not None and num != 0:
                pct = unit in ("%", "percent")
                suffix = "%" if pct else ""
                label = "" if pct or not unit else unit.upper()
                push(st + 0.2, 3.4, "BigNumber",
                     {"value": num, "suffix": suffix, "label": label,
                      "countup": True, "scrim": True}, "whoosh")
                placed = True

        # 3. A listy line -> FactCard, the channel signature panel.
        if not placed and used_facts < 4 and _enumerationish(text):
            parts = [x.strip() for x in text.replace(" and ", ", ").split(",")]
            items = [_clean(x, 46) for x in parts if len(x.strip()) >= 3][:5]
            if len(items) >= 3:
                push(st + 0.2, 5.0, "FactCard",
                     {"title": "KEY POINTS", "items": items, "variant": "panel", "side": "right"},
                     "whoosh")
                used_facts += 1
                placed = True

        # 4. Otherwise a news-style lower third carrying the actual sentence.
        if not placed:
            push(st + 0.2, 4.0, "NewsLowerThird",
                 {"kicker": "REPORT", "headline": _clean(text, 68)}, "")
            placed = True

        if placed:
            last_at = st
        if len(ov) >= max_overlays:
            break
...
def overlays(scenes):
    ...
    if key:
        ...
        try:
            ...
            # A thin AI track on a long video leaves big dead stretches; top it up.
            dur_total = segs[-1]["end"] if segs else 0
            want = max(6, int(dur_total / 25))
            if len(out["overlays"]) < want:
                have = sorted(float(o.get("start", 0)) for o in out["overlays"])
                for extra in auto_overlays(segs, dur_total):
                    if all(abs(extra["start"] - h) > 10 for h in have):
                        out["overlays"].append(extra)
                        have.append(extra["start"])
                        have.sort()
                    if len(out["overlays"]) >= want:
                        break
                out["overlays"].sort(key=lambda o: float(o.get("start", 0)))
                print(f"  topped up to {len(out['overlays'])} overlays")
    ...
    # No key (or the director failed): build a real overlay track from the narration rather
    # than shipping a single static title — that left all 47 animations unused.
    total = segs[-1]["end"] if segs else 0
    ovl = auto_overlays(segs, total)
    status("director", 68, f"auto overlays ({len(ovl)})")
    print(f"  auto-director wrote {len(ovl)} overlays")
